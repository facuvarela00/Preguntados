...
class Mustache_Test_Functional_InheritanceTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $partials = array(
            'head' => '|{{$stuff}}...{{/stuff}}{{$default}} default{{/default}}|',
        );

        $this->mustache->setPartials($partials);

        $tpl = $this->mustache->loadTemplate(
            'test {{<head}}{{$stuff}}override1{{/stuff}}{{/head}} {{<head}}{{$stuff}}override2{{/stuff}}{{/head}}'
        );
    ...
    public function testBlocksDoNotLeakBetweenPartials()
    {
        ...
        $tpl = $this->mustache->loadTemplate(
            'test {{<head}}{{$a}}C{{/a}}{{/head}} {{<head}}{{$b}}D{{/b}}{{/head}}'
        );
    ...
    public function testInheritIndentationWhenOverridingAPartial()
    {
        ...
        $tpl = $this->mustache->loadTemplate(
            '{{<head}}{{$nineties}}hammer time{{/nineties}}{{/head}}'
        );
    ...
    {
        $partials = array(
            'head' => '{{$stuff}}default one{{/stuff}}, {{$stuff2}}default two{{/stuff2}}',
        );

        $this->mustache->setPartials($partials);

        $tpl = $this->mustache->loadTemplate(
            '{{<head}}{{$stuff2}}override two{{/stuff2}}{{/head}}'
        );
